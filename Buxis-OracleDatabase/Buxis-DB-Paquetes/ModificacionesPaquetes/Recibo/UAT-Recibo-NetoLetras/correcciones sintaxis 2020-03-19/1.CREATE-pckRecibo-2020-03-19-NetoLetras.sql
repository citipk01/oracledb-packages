...
-- ----------------------------------------------------------------------------------------------
create or replace PACKAGE BODY	BASEARG.PKG_BXSCITI_DATOS_RECIBO
AS
/******************************************************************************
	NAME: BASEARG.PKG_BXSCITI_DATOS_RECIBO
	PURPOSE:
	...
	Ver Date Author Description
	--------- ---------- --------------- ------------------------------------
	1.0  30/04/2015 CSA 1.  Se procesan los datos para la 
						generacion de los Recibos de Sueldos
	
	1.1 2020-03-16 Copia el tamaño de la columna de CB_REC_MF.NETO_LETRAS
******************************************************************************/
	 ...
	 IS
			...
			l_reten            NUMBER (24, 8);
			l_neto             CB_REC_MF.NETO%TYPE; -- NUMBER (24, 8);
			l_neto_letras      CB_REC_MF.NETO_LETRAS%TYPE; 
				--VARCHAR2 (100);
			...
			IS
					 SELECT MHD.COD_MF,
						...
						WHERE     MHD.COD_LQ = p_cod_lq
									...
									-- EGV 18AGO2015 Inicio
									AND ACU.FECHA_EFECTIVA = (
											SELECT MAX(FECHA_EFECTIVA) 
											FROM ACUMULAN_TBL 
											WHERE ACU.ACUMULADOR = ACUMULADOR 
											AND FECHA_EFECTIVA <= c_fhas
											)
									AND ACU.BAJA_EFECTIVA IS NULL
									AND MCO.FECHA_EFECTIVA = (
										SELECT MAX(FECHA_EFECTIVA) 
										FROM MOVCOD_TBL 
										WHERE MCO.COD_MV = COD_MV 
										AND FECHA_EFECTIVA <= c_fhas
										)
	 ...
	 BEGIN
		 ...
		 para recibos de sueldo finalizo correctamente';
		 */
				 p_resultado :=
						'OK';         
				 -- EGV 18Ago2015 Fin
	 ...
	 BEGIN
			...
			*/
			...
			THEN
				 ...
				 END IF;
				 */
				 l_anos_ant_num := TRUNC(
		  MONTHS_BETWEEN (g_fec_hasta, g_fec_ant_mf) / 12);
 ...
 function numero_a_letras(p_numero in number) return varchar2 
		...
		begin
				...
				if millares_de_millon = 1 then
					...
					en_letras := PKG_BXSCITI_DATOS_RECIBO.numero_menor_mil(
						millares_de_millon);
				...
				if millones = 1 and  millares_de_millon = 0 then
						...
						en_letras := en_letras 
			  || PKG_BXSCITI_DATOS_RECIBO.numero_menor_mil(millones) 
			  || 'millones ';
				end if;
				...
				if millares = 1 and millares_de_millon = 0 and millones = 0 then
						...
						en_letras := en_letras 
			|| PKG_BXSCITI_DATOS_RECIBO.numero_menor_mil(millares) 
			|| 'mil ';
				end if;
				...
				if centenas > 0 or (entero = 0 and centimos = 0) then
						en_letras := en_letras 
			|| PKG_BXSCITI_DATOS_RECIBO.numero_menor_mil(centenas);
				end if;
				...
				if centimos > 0 then
				  ...
				  || 'con ' 
				  || replace(
					PKG_BXSCITI_DATOS_RECIBO.numero_menor_mil(centimos)
					,'uno ','un '
					) 
				  || aux;
						else
								en_letras := en_letras 
				|| replace(PKG_BXSCITI_DATOS_RECIBO.numero_menor_mil(centimos)
				  ,'uno','un'
				  ) 
				|| aux;
						end if;
	...
	function numero_menor_mil(p_numero in number) return varchar2 is
		...
		begin
				...
				select decode(a.signo,'-',-1,1)
						Into l_salida 
				from acumulan_tbl a
				where a.fecha_efectiva = (
		  select max(fecha_efectiva) 
		  from acumulan_tbl 
		  where acumulador = a.acumulador 
		  and fecha_efectiva <= g_fec_hasta
		  )
				and a.cod_mv = l_conc
				and a.acumulador = 'x9';
				exception
