   ...
   ******************************************************************************/
      ...
      IS
           ...
           SELECT MOV.COD_MF,
            ...
            WHERE MOV.COD_LQ = p_cod_lq
                  ...
                  AND to_char(MOV.COD_MF) IN (SELECT * FROM TABLE (c_legajos_tbl))
                  -- EGV 01Dic2015 Fin
                  AND EXISTS
                         (SELECT 1
                            -- EGV 15Jun2015 1.1 - Inicio
   ...
   BEGIN
      ...
      EXCEPTION
         ...
         THEN
            p_resultado :=
               'Ha ocurrido un error obteniendo los datos para el codigo de liquidacion '
               || p_cod_lq
               || '.';
      ...
      --> Busco los empleados involucrados dependiendo de los parametros ingresados
      ...
      CASE p_rep_errores
         ...
         WHEN 'C'
         THEN --> Se procesa/reprocesa los legajos que vienen en el parametro p_legajos
            l_legajos_tbl :=
      ...
      EXCEPTION
         ...
         THEN
            p_resultado :=
               'Error procesando la cabecera de los asientos para la liquidacion : '
               || g_cod_lq;
      ...
      BEGIN
         --> Se deben borrar los registros de la CB_ASIENTO corrsepondientes los empleados procesados y los asientos de ajuste.
        ...
        -- EGV 01Dic2015 Fin
             DELETE FROM BASEARG.CB_ASIENTO A
                   WHERE A.COD_LQ = p_cod_lq
                         -- EGV 01Dic2015 Inicio
                         --AND A.COD_MF IN (SELECT * FROM TABLE (l_legajos_tbl));
                         AND to_char(A.COD_MF) IN (SELECT * FROM TABLE (l_legajos_tbl));
                         -- EGV 01Dic2015 Fin
                         

             DELETE FROM BASEARG.CB_ASIENTO A
                   WHERE A.COD_LQ = p_cod_lq AND A.CUENTA LIKE 'AJUSTE00099999%';
        -- EGV 01Dic2015 Inicio
        END IF;
        -- EGV 01Dic2015 Fin
      EXCEPTION
      ...
      LOOP
         ...
         LOOP
            BEGIN
               ...
               THEN
                  g_descr_det :=
                     'Existen datos inconsistentes en la imputacion contable para el concepto : '
                     || l_cod_mv_t (i)
                     || ' empleado : '
                     || l_cod_mf_t (i);
            ...
            EXCEPTION
               ...
               THEN
                  ...
                  /*g_descr_det :=
                     ...
                     || ' Concepto : '
                     || l_cod_mv_t (i);*/
                   --g_descr_det := 'Error con Legajo: ' || l_cod_mf_t (i) || ' Concepto: ' || l_cod_mv_t(i) || ' ' || substr(replace(replace(sqlerrm,chr(10),'-'),chr(13),'-'),1,100);
                   g_descr_det := 'Error con Legajo: ' || l_cod_mf_t (i) || ' Concepto: ' || l_cod_mv_t(i) || ' Error:' || sqlcode;
                   -- EGV 09Dic2015 Fin
   ...
   BEGIN
      ...
      THEN
         --> Obtener Cuenta Cajero . Si no tiene cuenta no se procesa el registro
      ...
      ELSE
         ...
         SELECT IC.CUENTA,
          ...
          -- EGV 1.1 15Jun2015 Fin
                ...
                AND rtrim(IC.BANKING,' ') = g_banking_mf
