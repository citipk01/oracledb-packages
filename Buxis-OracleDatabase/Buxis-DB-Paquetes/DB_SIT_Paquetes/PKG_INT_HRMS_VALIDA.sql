...
/******************************************************************************
 ...
 PURPOSE: Valida la informacion de las tablas de Interfaz y arma la tabla CB_MAEFUNC_STG
...
/** 0140**/ PROCEDURE Valida_CB_PERS_NID_INT (l_res OUT VARCHAR2);
 ...
 PROCEDURE Obtener_Tipo_Trans (l_audit_action IN VARCHAR2,l_tipo_tran OUT VARCHAR2);
 -- EGV 12FEB2016 Inicio
 --PROCEDURE Armar_Cod_Mf (l_emplid IN VARCHAR2, l_empl_rcd VARCHAR2, l_cod_mf OUT INTEGER);
 PROCEDURE Armar_Cod_Mf (l_emplid IN VARCHAR2, l_empl_rcd VARCHAR2, l_region VARCHAR2, l_cod_mf OUT INTEGER);
 -- EGV 12FEB2016 Fin
 -- EGV 01Jul2015 - Inicio
 --PROCEDURE Buscar_Emplid (l_emplid IN VARCHAR2, l_empl_rcd VARCHAR2,l_cod_mf OUT INTEGER);
 PROCEDURE Buscar_Emplid (l_emplid IN VARCHAR2, l_empl_rcd VARCHAR2,l_region VARCHAR2, l_cod_mf OUT INTEGER);
 PROCEDURE TraeRegionDeEmplid(l_emplid IN VARCHAR2, l_empl_rcd IN INTEGER, l_effdt IN DATE, l_effseq IN INTEGER, l_region OUT VARCHAR2);
 -- EGV 01Jul2015 - Fin
 -- EGV 01Jul2015 - Inicio
 --PROCEDURE Buscar_reg_MAEFUNC_STG (l_cod_int IN INTEGER,l_cod_mf IN INTEGER ,l_effdt IN DATE ,l_effseq IN INTEGER,l_tipo VARCHAR2,l_reg OUT g_mf_list,l_existe OUT INTEGER);
 PROCEDURE Buscar_reg_MAEFUNC_STG (l_cod_int IN INTEGER,l_cod_mf IN INTEGER ,l_effdt IN DATE ,l_effseq IN INTEGER,l_tipo VARCHAR2,l_emplid VARCHAR2,l_reg OUT g_mf_list,l_existe OUT INTEGER);
 -- EGV 01Jul2015 - Fin 
 PROCEDURE Buscar_reg_MAEFUNC_TBL (l_cod_int IN INTEGER,l_cod_mf IN INTEGER ,l_effdt IN DATE ,l_effseq IN INTEGER,l_tipo VARCHAR2,l_reg OUT g_mf_list,l_existe OUT INTEGER);
 PROCEDURE Impacta_reg_MAEFUNC_STG(l_accion IN VARCHAR2,l_reg IN g_mf_list,l_res OUT VARCHAR2);
 PROCEDURE Valida_reg_MAEFUNC_STG (l_cod_int IN INTEGER,l_res OUT VARCHAR2);
 
-- EGV 22SEP2015 Inicio
 FUNCTION CalculaNuevaAccion(l_action VARCHAR2, l_action_reason VARCHAR2, l_old_action VARCHAR2, l_emplid VARCHAR2, l_empl_rcd INTEGER, l_effdt DATE, l_effseq INTEGER) RETURN VARCHAR2;
...
--  BASEARG.PKG_BXSCITI_GENERAL.GRABAR_LOG_DET (g_cod_int, g_proc_int, 'I', 'Mensaje de log', g_res_log);
...
-- EGV 01Jul2015 - Inicio
 ...
 AND A.NRO_LINEA = (SELECT MAX(NRO_LINEA) FROM CB_JOB_INT WHERE A.COD_INTFC =
      COD_INTFC
      AND A.EMPLID = EMPLID
      AND A.EMPL_RCD = EMPL_RCD
      AND A.EFFDT = EFFDT
...
-- EGV 22SEP2015 Inicio
 ...
 BEGIN
    ...
    AND A.FECHA_EFECTIVA = (SELECT MAX(B.FECHA_EFECTIVA) FROM CB_GRADO B WHERE
      A.GRADO = B.GRADO
...
l_new_empl_rcd VARCHAR2(3) := ' ';
 ...
 FROM MAEFUNC_TBL M
 WHERE trim(M.CB_GEID) = l_emplid
 AND (to_char(M.CB_EMPL_RCD) = l_empl_rcd OR l_empl_rcd = ' ')
 UNION
 SELECT DISTINCT MS.COD_MF
 FROM CB_MAEFUNC_STG MS
 WHERE MS.COD_INTFC = g_cod_int
  AND trim(MS.CB_GEID) = l_emplid
 AND (to_char(MS.CB_EMPL_RCD) = l_empl_rcd OR l_empl_rcd = ' ')) MM;
 ...
 IF l_cod_mf = 0 AND l_region = 'ARG' AND l_empl_rcd <> ' ' AND l_empl_rcd <> '0' THEN
 BEGIN 
    l_new_empl_rcd := '0';
 -- EGV 12FEB2016 Fin
 ...
 WHERE trim(M.CB_GEID) = l_emplid
 AND (to_char(M.CB_EMPL_RCD) = l_new_empl_rcd OR l_new_empl_rcd = ' ')
 -- EGV 26Jun2015 - Fin
 UNION
 SELECT DISTINCT MS.COD_MF
 FROM CB_MAEFUNC_STG MS
 ...
 --AND MS.CB_GEID = l_emplid
  ...
  AND trim(MS.CB_GEID) = l_emplid
 AND (to_char(MS.CB_EMPL_RCD) = l_new_empl_rcd OR l_new_empl_rcd = ' ')) MM;
 -- EGV 26Jun2015 - Fin
 EXCEPTION
    WHEN NO_DATA_FOUND
    THEN
 ...
 BEGIN
 /* l_tipo :
 FSE+ : Busca todos los registros de la fecha efectiva / seq efectiva pasada por parametro para adelante.
 Si no existe esa fecha/sequencia busca el registro anterior ( si existe) el que sera usado para cambiarle los datos de fecha/sequencia e insertarlo como nuevo registro.
 FE+ :Busca todos los registros de la fecha efectiva pasada por parametro para adelante.
 TO : Busca todos los registros correspondiente al empleado pasado por parametro
 TO+ : Busca todos los registros correspondiente al empleado pasado por parametro sin importar la empresa. Se compara directamente con el CB_GEID
 ...
 */
 ...
 CASE l_tipo
     ...
     THEN
         ...
         IF l_existe = 2
         THEN
      -- Hay registros modificados x Buxis. Tengo que buscar el registro anterior para tomar como base de actualizacion.
             BEGIN
             SELECT DISTINCT 1
             INTO l_existe_2
             FROM CB_MAEFUNC_STG S
             ...
             IF l_existe_2 = 1
             THEN
      -- Existe esa fecha efectiva pero con unas EFFSEQ menor. traigo ese registro + el registro a eliminar
                 SELECT S.*
                 BULK COLLECT INTO l_reg
                 FROM CB_MAEFUNC_STG S
                 WHERE S.COD_INTFC = l_cod_int AND S.COD_MF = l_cod_mf
     ...
     THEN
         ...
         ELSE -- NO Existe la clave en la tabla
             ...
             THEN -- Existe esa fecha efectiva pero con unas EFFSEQ menor
                 ...
                 S.SEC_EFECTIVA;
             ELSE
      -- NO Existe la clave en la tabla y NO Existe esa fecha efectiva pero con unas EFFSEQ menor
                 SELECT S.*
                 BULK COLLECT INTO l_reg
                 FROM CB_MAEFUNC_STG S
                 WHERE S.COD_INTFC = l_cod_int AND S.COD_MF = l_cod_mf
 ...
 */
     ...
     THEN
         ...
         ELSE -- NO Existe la clave en la tabla
             ...
             AND M.SEC_EFECTIVA = (SELECT MAX(M3.SEC_EFECTIVA)
                                    FROM MAEFUNC_TBL M3
                                        WHERE M.COD_MF = M3.COD_MF
                                        AND M.FECHA_EFECTIVA = M3.
      FECHA_EFECTIVA
                                        AND ((M3.FECHA_EFECTIVA = l_effdt AND
      M3.SEC_EFECTIVA <= l_effseq)
      ...
      l_from_effseq) OR S.FECHA_EFECTIVA > l_from_effdt)
                AND (S.CB_HRMS_EFFDT < l_effdt OR (S.CB_HRMS_EFFDT = l_effdt
      AND S.CB_HRMS_EFFSEQ <= l_effseq))
                UNION
                SELECT l_cod_int,
                  M.*,
                ...
                FROM MAEFUNC_TBL M
                WHERE M.COD_MF = l_cod_mf
                AND ((M.FECHA_EFECTIVA = l_from_effdt AND M.SEC_EFECTIVA >=
      l_from_effseq) OR M.FECHA_EFECTIVA > l_from_effdt)
                AND (M.CB_HRMS_EFFDT < l_effdt OR (M.CB_HRMS_EFFDT = l_effdt
      AND M.CB_HRMS_EFFSEQ <= l_effseq))
     ...
     LOOP
         ...
         EXCEPTION
             ...
             THEN
                 ...
                 || TRIM (t_mf (X).CENCOS_MF) ||  ' para la empresa ' || t_mf (X).COD_EMP;
                 BASEARG.PKG_BXSCITI_GENERAL.GRABAR_LOG_DET (l_cod_int,
                 g_proc_int,
                 'E',
                 g_err_descr,
      ...
      t_mf (X).COD_EMP || ' no invalida registro.';
         ...
         THEN
             ...
             l_det
             ||
      ' --> No pueden estar en Nulo los siguientes campos : COD_MF , COD_EMP , NIVEL , PRI_NOM_MF , PRI_APE_MF '
      ;
             BASEARG.PKG_BXSCITI_GENERAL.GRABAR_LOG_DET (l_cod_int,
             g_proc_int,
             'E',
 ...
 l_res VARCHAR2 (500);
 BEGIN
 --SE EJECUTAN LAS VALIDACIONES CORRESPONDIENTE Y SE LLENAN LAS TABLAS DE STAGING
 g_cod_int := Cod_int;
 l_error := FALSE;
 ...
 LOOP
     ...
     FROM QSEMPRESA E
     WHERE TRIM (E.COMPANY) = case when J.COMPANY in ('UUY','CUY','FUY','WUY')
      then J.COMPANY
     ...
     THEN --Es una Eliminacion
                     ...
                     LOOP
                         ...
                         THEN
                             ...
                             THEN
                                ...
                                -- EGV 20Dic2016 Fin                                    
                             ELSE
                                 t_mf (X).CB_DEPT := t_mf (l_index).CB_DEPT;
                                 t_mf (X).COD_CATE := t_mf (l_index).COD_CATE;
                                 t_mf (X).ACCION := t_mf (l_index).ACCION;
                                 t_mf (X).ACCION_DT :=
                                 ...
                                 t_mf (X).CONTRATO := t_mf (l_index).CONTRATO;
                                 t_mf (X).COD_EMP := t_mf (l_index).COD_EMP;
                                 t_mf (X).TIPOREM_MF :=
                                 t_mf (l_index).TIPOREM_MF;
                                 t_mf (X).SJH_MF := t_mf (l_index).SJH_MF;
                                 -- EGV 15SEP2015 Inicio
                                 --t_mf (X).SINDICATO :=
                                 --t_mf (l_index).SINDICATO;
                                 IF l_REGION = 'URY' THEN
                                    t_mf (X).SINDICATO := t_mf (l_index).
      SINDICATO;
                                 END IF;
                                 -- EGV 15SEP2015 Fin
                                 t_mf (X).SECC_MF := t_mf (l_index).SECC_MF;
                                 t_mf (X).CENCOS_MF :=
                                 t_mf (l_index).CENCOS_MF;
                                 t_mf (X).BANKING := t_mf (l_index).BANKING;
                                 t_mf (X).GRADO := t_mf (l_index).GRADO;
                                 t_mf (X).STD_HOURS := t_mf (l_index).STD_HOURS; -- EGV 15SEP2015
                             END IF;
                         END IF;
                     END LOOP;

                     t_mf.DELETE (l_index);
      -- Borro el registro tomado como base para la actualizacion de los registros con CB_BX_CHANGE=1
         ...
         ELSE -- No Existen registros en la tabla MAEFUNC_STG para procesar
             ...
             THEN --Existen registros en la tabla MAEFUNC_TBL para procesar
                 ...
                 THEN -- No hay registros modificados x Buxis
                     IF t_mf.COUNT > 0
                     THEN
      --Existen registros en la tabla MAEFUNC_TBL para eliminar
                         FOR X IN t_mf.FIRST .. t_mf.LAST
                         LOOP
                            t_mf (X).TIPO_TRAN := 'D';
                         END LOOP;
      ...
      SINDICATO;
                                     END IF;
                                     -- EGV 15SEP2015 Fin
                                     t_mf (X).SECC_MF := t_mf (l_index).
      SECC_MF;
                                     t_mf (X).CENCOS_MF :=
                                     t_mf (l_index).CENCOS_MF;
                                     t_mf (X).BANKING := t_mf (l_index).
      BANKING;
                                     t_mf (X).GRADO := t_mf (l_index).GRADO;
                                     t_mf (X).STD_HOURS := t_mf (l_index).STD_HOURS;-- EGV 15SEP2015
                                 END IF;
      ...
      -- Borro el registro tomado como base para la actualizacion de los registros con CB_BX_CHANGE=1
            ...
            ELSE -- Existe = 0 en MAEFUNC_TBL
                 ...
                 t_mf (1).ACCION := CalculaNuevaAccion(J.ACTION, J.
      ACTION_REASON, t_mf (1).ACCION, J.EMPLID, J.EMPL_RCD, J.EFFDT, J.EFFSEQ)
      ;
                 -- EGV 22SEP2015 Fin
                 ...
                 t_mf (1).CONTRATO := CASE J.EMPL_CLASS WHEN 'M' THEN 'P' ELSE J.REG_TEMP END;
                 -- EGV 19Jun2017 Fin
                 ...
                 t_mf (1).GRADO := CASE J.EMPL_CLASS WHEN 'M' THEN '4' ELSE J.CG_MAPPEDGRADE END;
                 -- EGV 23Ago2017 Fin
                 t_mf (1).CUOTA_CLUB := TraeCuotaClub(t_mf (1).GRADO);
      -- EGV 22SEP2015
                 t_mf (1).CAT_CONT := TraeCatCont(t_mf (1).GRADO);
      -- EGV 05OCT2015
                 t_mf (1).BANKING := TraeBanking(t_mf (1).CENCOS_MF, t_mf (1).
      COD_EMP);     -- EGV 05OCT2015
     ...
     ELSE --NO Es una Eliminacion
      ...
      COD_EMP);     -- EGV 05OCT2015
                 ...
                 -- EGV 23Ago2017 Inicio
                 t_mf (1).MODCON := CASE J.EMPL_CLASS WHEN 'M' THEN '27' ELSE '8' END;
                 ...
                 LOOP
                     ...
                     THEN
                         ...
                         THEN
                             ...
                             ELSE
                                 g_err_descr :=
                                 'Existe una inconsistencia con el registro a procesar 1'
                                 || ' COD_MF : '
                                 || t_mf (X).COD_MF
                                 || ' FECHA_EFF : '
                     ...
                     */
                     ...
                     THEN
                        ...
                        --if t_mf(X).FECHA_EFECTIVA <> J.EFFDT or t_mf (X).SEC_EFECTIVA <> J.EFFSEQ then
                        if t_mf(X).FECHA_EFECTIVA < J.EFFDT or (t_mf(X).
      FECHA_EFECTIVA = J.EFFDT
      and t_mf (X).SEC_EFECTIVA < J.EFFSEQ)
      then
                             t_mf (X).FECHA_EFECTIVA := J.EFFDT;
                             t_mf (X).SEC_EFECTIVA := J.EFFSEQ;
                             --t_mf (X).CB_EMPL_RCD := J.EMPL_RCD;
                     ...
                     t_mf (X).ACCION := CalculaNuevaAccion(J.ACTION, J.
      ACTION_REASON, t_mf (X).ACCION, J.EMPLID, J.EMPL_RCD, J.EFFDT, J.EFFSEQ)
      ;
                     -- EGV 22SEP2015 Fin
                     -- EGV 26JUL2016 Inicio
                     ...
                     t_mf (X).CONTRATO := CASE J.EMPL_CLASS WHEN 'M' THEN 'P' ELSE J.REG_TEMP END;
                     -- EGV 19Jun2017 Fin
                     ...
                     t_mf (X).GRADO := CASE J.EMPL_CLASS WHEN 'M' THEN '4' ELSE J.CG_MAPPEDGRADE END;
                     -- EGV 23Ago2017 Fin
                     t_mf (X).CUOTA_CLUB := TraeCuotaClub(t_mf (X).GRADO);
      -- EGV 22SEP2015
                     t_mf (X).CAT_CONT := TraeCatCont(t_mf (X).GRADO);
      -- EGV 05OCT2015
                     t_mf (X).BANKING := TraeBanking(t_mf (X).CENCOS_MF, t_mf
      (X).COD_EMP);     -- EGV 05OCT2015
                     -- EGV 24NOV2015 Inicio
             ...
             LOOP
                 ...
                 THEN
                     ...
                     THEN
                         ...
                         ELSE
                             g_err_descr :=
                             'Existe una inconsistencia con el registro a procesar 2'
                             || ' COD_MF : '
                             || t_mf (X).COD_MF
                             || ' FECHA_EFF : '
                             || J.EFFDT     --|| t_mf (X).FECHA_EFECTIVA
      ...
      -- EGV 22SEP2015
                 t_mf (X).CAT_CONT := TraeCatCont(t_mf (X).GRADO);
      -- EGV 05OCT2015
                 t_mf (X).BANKING := TraeBanking(t_mf (X).CENCOS_MF, t_mf (X).
      COD_EMP);     -- EGV 05OCT2015
                 -- EGV 24NOV2015 Inicio
 ...
 LOOP
     ...
     THEN --No Es una Eliminacion
         ...
         THEN --Existen registros en la tabla MAEFUNC_STG para procesar
            ...
            IF (nvl(rtrim(t_mf(l_iLast).PRI_APE_MF),' ') <> nvl(rtrim(
      l_last_name),' ') OR nvl(rtrim(t_mf(l_iLast).PRI_NOM_MF),' ') <> nvl(
      rtrim(l_first_name),' ') OR nvl(rtrim(t_mf(l_iLast).SEG_NOM_MF),' ') <>
      nvl(rtrim(l_middle_name),' ') OR nvl(rtrim(t_mf(l_iLast).SEG_APE_MF),' '
 ...
 LOOP
     ...
     THEN --No Es una Eliminacion
         ...
         THEN --Existen registros en la tabla MAEFUNC_STG para procesar
            -- EGV 25Ago2015 Inicio
            l_iLast := t_mf.LAST;
            IF ((PN.PRIMARY_NID = 'Y' AND (nvl(rtrim(t_mf(l_iLast).TIPO_DOC),
      ' ') <> nvl(rtrim(PN.NATIONAL_ID_TYPE),' ') OR nvl(rtrim(t_mf(l_iLast).
      CEDIDE_MF),' ') <> nvl(rtrim(PN.NATIONAL_ID),' '))) OR (PN.
      NATIONAL_ID_TYPE = 'CUIL'
      AND nvl(rtrim(t_mf(l_iLast).CUIL),' ') <> nvl(rtrim(PN.NATIONAL_ID),' ')
      ))
      then
            -- EGV 25Ago2015 Fin
             FOR X IN t_mf.FIRST .. t_mf.LAST
 ...
 FROM CB_MAEFUNC_STG
   -- WHERE trim(CB_GEID) = l_geid
 WHERE ltrim(CB_GEID, '0') = l_geid --* 2019 04 17 *: para legajos que comienzan con ceros AND COD_INTFC = g_cod_int;
 -- EGV 12FEB2016 Fin
 ...
 LOOP
     ...
     THEN --No Es una Eliminacion
         ...
         LOOP
            ...
            ELSE
                ...
                IF l_existe_soeid = 1 THEN
                    INSERT INTO CB_SOEID_STG(COD_INTFC, COD_MF, SOEID, ESTADO_STG, COD_ERROR, TIPO_TRAN)
     ...
     -- Busco si existen registros en MAEFUNC_STG
         ...
         THEN --Existen registros en la tabla MAEFUNC_STG para procesar
            ...
            -- EGV 25Ago2015 Inicio
            l_iLast := t_mf.LAST;
            IF (nvl(t_mf(l_iLast).FECNAC_MF,'01-jan-1800') <> nvl(P.BIRTHDATE,
      '01-jan-1800') OR nvl(rtrim(t_mf(l_iLast).NACIONALIDAD),' ') <> nvl(
      rtrim(P.BIRTHCOUNTRY),' '))
      THEN
            -- EGV 25Ago2015 Fin
 ...
 LOOP
     ...
     THEN --No Es una Eliminacion
         ...
         THEN --Existen registros en la tabla MAEFUNC_STG para procesar
            ...
            -- EGV 14OCT2015 Inicio - Si la antiguedad reconocida es nula, asignarle el menor
            IF E.PROF_EXPERIENCE_DT IS NULL THEN
                E.PROF_EXPERIENCE_DT := LEAST(E.HIRE_DT,E.CMPNY_SENIORITY_DT,E.SERVICE_DT); 
            END IF;
            -- EGV 14OCT2015 Fin
            
            
            -- EGV 25Ago2015 Inicio
            -- Se compara la última fila de MAEFUNC contra lo que vino por interfaz
            -- Para los campos de que se impactan de EMPLOYMENT
            -- Si hay diferencias se procesa, sino no
            -- Se toma la última fila de MAEFUNC sólo por elegir una
            -- ya que estos campos deberían ser idénticos en todas las fechas
            -- El campo qeu varía de fecha a fecha es el de fecha de baja, por eso también se toma la última maefunc
            l_iLast := t_mf.LAST;
            if (nvl(t_mf (l_iLast).FEC_DESV_MF,'01-jan-1800') <> nvl(E.
      TERMINATION_DT,'01-jan-1800') OR nvl(t_mf(l_iLast).FEC_HIRE,
      '01-jan-1800') <> nvl(E.HIRE_DT,'01-jan-1800') OR nvl(t_mf(l_iLast).
      FEC_REHIRE,'01-jan-1800') <> nvl(E.REHIRE_DT,'01-jan-1800') OR nvl(t_mf(
      l_iLast).FEC_ING_MF,'01-jan-1800') <> nvl(E.CMPNY_SENIORITY_DT,
      '01-jan-1800') OR nvl(t_mf(l_iLast).FEC_ANTIG,'01-jan-1800') <> nvl(E.
      SERVICE_DT,'01-jan-1800') OR nvl(t_mf(l_iLast).FEC_INDEM_MF,
      '01-jan-1800') <> nvl(E.PROF_EXPERIENCE_DT,'01-jan-1800') OR nvl(rtrim(
      t_mf(l_iLast).BUSINESS_TITLE),' ') <> nvl(rtrim(E.BUSINESS_TITLE),' '))
            THEN
            -- EGV 25Ago2015 Fin
             FOR X IN t_mf.FIRST .. t_mf.LAST
             LOOP

                 -- EGV 22SEP2015 Inicio Al final la fecha de baja la afectaremos a todos los registros
                 ...
                 --            l_index := X;
                 --        ELSE
                 --            IF (E.TERMINATION_DT - t_mf (X).FECHA_EFECTIVA) <
                 --            (E.TERMINATION_DT
                 ...
                 --   t_mf (l_index).FEC_EGR_MF := E.TERMINATION_DT;-- EGV 25Ago2015 Inicio
                 --END IF;
 ...
 LOOP
     ...
     -- EGV 20Dic2016 Inicio  No se procesan las Eliminaciones, ya que se procesan con la Tabla JOB
     /*
     IF l_t_trans = 'D'
     THEN --Es una Eliminacion
         -- Busco si existen registros en MAEFUNC_STG
 ...
 g_proc_int,
 'I',
 'Inicio de proceso x: ' || l_cod_mf,
 g_res_log);

     IF l_t_trans <> 'D'
     THEN -- NO es una Eliminación
     -- EGV 20Dic2016 Fin
         -- Busco si existen registros en MAEFUNC_STG
         BASEARG.PKG_INT_HRMS_VALIDA.
         BUSCAR_REG_MAEFUNC_STG (J.COD_INTFC,
         ...
         THEN --NO existen registros en la tabla MAEFUNC_STG
             ...
             -- EGV 20Dic2016 Inicio -- No se inserta más desde JOB_JR, solo desde JOB
            ...
            /*
                 g_err_descr :=
                 'Se esta cargando un registro JOB_JR para una fecha/sec inexsitente '
                 || ' COD_MF : '
                 || l_cod_mf
                 || ' FECHA_EFF : '
                 || J.EFFDT
             ...
             THEN 
                ...
                -- EGV 20Dic2016 Fin
                     ...
                     LOOP
                         ...
                         -- 04OCT2018 Begin // JEV 09OCT2018 Inicio - Se elimina campo cgar_qsosocial
                        -- t_mf (x).osocial_citi := J.CGAR_QSOSOCIAL;
                        ...
                        -- 04OCT2018 End   // JEV 09OCT2018 Inicio - Se elimina campo cgar_qsosocial
             ...
             LOOP
                 ...
                 THEN
                     ...
                     THEN
                         ...
                         ELSE
                             g_err_descr :=
                             'Existe una inconsistencia con el registro a procesar 4'
                             || ' COD_MF : '
                             || t_mf (X).COD_MF
                             || ' FECHA_EFF : '
                             || J.EFFDT     --|| t_mf (X).FECHA_EFECTIVA
 ...
 LOOP
     ...
     */
     ...
     -- EGV 20Dic2016 Fin 
        ...
        -- EGV 20Dic2016 Inicio -- tratar a PERS_DT de manera similar a PERS
        --osea no tener en cuenta la fecha efectiva y actualizar todos los registros 
        /*
        -- Busco si existen registros en MAEFUNC_STG
         BASEARG.PKG_INT_HRMS_VALIDA.
         BUSCAR_REG_MAEFUNC_STG (J.COD_INTFC,
                 ...
                 LOOP
                     ...
                     THEN
                         ...
                         THEN
                             ...
                             ELSE
                                 g_err_descr :=
                                 'Existe una inconsistencia con el registro a procesar 5'
                                 || ' COD_MF : '
                                 || t_mf (X).COD_MF
                                 || ' FECHA_EFF : '
                                 || J.EFFDT     --|| t_mf (X).FECHA_EFECTIVA
                 ...
                 LOOP
                     ...
                     THEN
                         ...
                         THEN
                             ...
                             ELSE
                                 g_err_descr :=
                                 'Existe una inconsistencia con el registro a procesar 6'
                                 || ' COD_MF : '
                                 || t_mf (X).COD_MF
                                 || ' FECHA_EFF : '
                                 || J.EFFDT     --|| t_mf (X).FECHA_EFECTIVA
        ...
        THEN --Existen registros en la tabla MAEFUNC_STG para procesar
            ...
            l_iLast := t_mf.LAST;
            IF (nvl(rtrim(t_mf(l_iLast).EST_CIV_MF),' ') <> nvl(rtrim(J.MAR_STATUS),' ')
                OR nvl(rtrim(t_mf(l_iLast).SEXO_MF),' ') <> nvl(rtrim(J.SEX),' '))
            THEN
...
-- Osea sin tener en cuenta la fecha efectiva, se actualizan todos los registros
/*
 -- Busco si existen registros en MAEFUNC_STG
 BASEARG.PKG_INT_HRMS_VALIDA.
 BUSCAR_REG_MAEFUNC_STG (J.COD_INTFC,
 ...
 */
        ...
        THEN --Existen registros en la tabla MAEFUNC_STG para procesar
            ...
            l_iLast := t_mf.LAST;
            IF (nvl(rtrim(t_mf(l_iLast).PAIS_MF),' ') <> nvl(rtrim(J.COUNTRY),' ')
                OR nvl(rtrim(t_mf(l_iLast).DOMIC_MF),' ') <> nvl(rtrim(J.ADDRESS1 || ' ' || J.ADDRESS2 || ' ' || J.ADDRESS3 || ' ' || J.ADDRESS4),' ')
                OR nvl(rtrim(t_mf(l_iLast).LOCALIDAD),' ') <> nvl(rtrim(J.CITY),' ')
                OR nvl(rtrim(t_mf(l_iLast).PROVINCIA),' ') <> nvl(rtrim(J.STATE),' ')
                OR nvl(rtrim(t_mf(l_iLast).CODPOS),' ') <> nvl(rtrim(J.POSTAL),' ')
            ...
            THEN
                ...
                LOOP
                    t_mf (X).PAIS_MF := J.COUNTRY;
                    t_mf (X).DOMIC_MF := J.ADDRESS1 || ' ' || J.ADDRESS2 || ' ' || J.ADDRESS3 || ' ' || J.ADDRESS4;
                     t_mf (X).LOCALIDAD := J.CITY;
                     t_mf (X).PROVINCIA := J.STATE;
                     t_mf (X).CODPOS := J.POSTAL;
                END LOOP;
